...
HEALTHCHECK CMD if [ "$(dig example.com IN A +short @127.0.0.1 | head -n 1)" != "$(dig example.com IN A +short +tls @"$dns" | head -n 1)" ]; then kill "$(cat /usr/local/etc/unbound/unbound.pid)"; fi || exit 1
